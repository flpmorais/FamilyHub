...
export function RecipeAddModal({ visible, onClose }: RecipeAddModalProps) {
  ...
  async function handleExtractUrl() {
    ...
    try {
      ...
      setErrorMsg(err instanceof Error ? err.message : 'Não foi possível extrair a receita.');
    } finally {
      setIsExtracting(false);
    }
  }
  ...
  async function handleExtractPhoto() {
    ...
    setErrorMsg('');
    try {
      const extracted = await recipeImportRepo.extractFromPhoto(imageBase64, imageMimeType, model);
      ...
      setErrorMsg(err instanceof Error ? err.message : 'Não foi possível extrair a receita da foto.');
    } finally {
      setIsExtracting(false);
    }
  }
  ...
  async function handleExtractText() {
    ...
    try {
      ...
      setErrorMsg(err instanceof Error ? err.message : 'Não foi possível extrair a receita do texto.');
    } finally {
      setIsExtracting(false);
    }
  }
      ...
      <View style={s.modelSection}>
        ...
        <View style={s.modelRow}>
          {MODEL_OPTIONS.map((opt) => (
            ...
            >
              <Text style={[s.modelChipText, model === opt.key && s.modelChipTextActive]}>
                {opt.label}
              </Text>
              <Text style={[s.modelChipDesc, model === opt.key && s.modelChipDescActive]}>
                {opt.desc}
              </Text>
            </TouchableOpacity>
          ))}
  ...
  function renderUrlStep() {
    ...
    return (
      <>
        ...
        {isYoutube && (
          <Text style={s.youtubeHint}>YouTube detetado</Text>
        )}
        ...
        <TouchableOpacity
          style={[s.extractBtn, (!url.trim() || isExtracting) && s.extractBtnDisabled]}
  ...
  function renderTextStep() {
    return (
      <ScrollView bounces={false} showsVerticalScrollIndicator={false}>
        ...
        <TouchableOpacity
          style={[s.extractBtn, (!rawText.trim() || isExtracting) && s.extractBtnDisabled]}
