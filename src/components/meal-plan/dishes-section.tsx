...
export function DishesSection({
  ...
  async function handleRemove(dishId: string, dishName: string) {
    Alert.alert("Remover prato", `Remover "${dishName}" desta refeição?`, [
  ...
  return (
    <View style={s.container}>
      ...
      {dishes.map((dish) => {
        ...
        const showServings =
          dish.dishType === "recipe" && dish.servingsOverride !== null;
              ...
              {showServings && (
                <View style={s.servingsRow}>
                  <TouchableOpacity
                    style={s.servingsBtn}
                    onPress={() =>
                      handleServingsChange(dish.id, dish.servingsOverride!, -1)
                    }
                  >
