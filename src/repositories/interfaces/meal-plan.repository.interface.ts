import { MealEntry, MealEntryDish, CreateMealEntryInput, UpdateMealEntryInput, CreateDishInput, MealPlanSlotConfig, MealSlot } from '../../types/meal-plan.types';
...
export interface IMealPlanRepository {
  ...
  // Dish operations
  getDishesForEntries(entryIds: string[]): Promise<Map<string, MealEntryDish[]>>;
  addDishes(mealEntryId: string, dishes: CreateDishInput[]): Promise<MealEntryDish[]>;
  removeDish(dishId: string): Promise<void>;
  updateDishServings(dishId: string, servings: number): Promise<void>;
  getRecentDishes(familyId: string, beforeDate: string, daysBack: number): Promise<MealEntryDish[]>;
}
