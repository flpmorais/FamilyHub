...
export class SupabaseTaskTemplateRepository implements ITaskTemplateRepository {
  ...
  private async saveProfiles(templateId: string, profileIds: string[]): Promise<void> {
    const { error: delError } = await this.client
      .from('task_template_profiles')
      .delete()
      .eq('task_template_id', templateId);
  ...
  async getTaskTemplates(familyId: string): Promise<TaskTemplate[]> {
    try {
      ...
      return rows.map((row: any) =>
        mapTaskTemplate(row, tagsByTpl.get(row.id) ?? [], profilesByTpl.get(row.id) ?? [])
      );
    } catch (err) {
  ...
  async createTaskTemplate(familyId: string, input: CreateTaskTemplateInput): Promise<TaskTemplate> {
    ...
    try {
      ...
      if (input.tagIds && input.tagIds.length > 0) await this.saveTags(id, input.tagIds);
      if (input.profileIds && input.profileIds.length > 0) await this.saveProfiles(id, input.profileIds);
  ...
  async updateTaskTemplate(
    ...
    if (data.deadlineDays !== undefined) updates.deadline_days = data.deadlineDays;
    ...
    try {
      ...
      if (data.profileIds !== undefined) await this.saveProfiles(id, data.profileIds);
