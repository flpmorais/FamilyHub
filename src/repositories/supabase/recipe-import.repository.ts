...
import {
  IRecipeImportRepository,
  type LlmModel,
} from "../interfaces/recipe-import.repository.interface";
import type { ExtractedRecipe, RecipeType } from "../../types/recipe.types";
import { logger } from "../../utils/logger";

const VALID_TYPES: RecipeType[] = [
  "meal",
  "main",
  "side",
  "soup",
  ...
  async extractFromUrl(
    ...
    try {
      ...
      logger.info(
        ...
        `invoke result: error=${error?.constructor?.name ?? "none"}, data=${JSON.stringify(data)?.slice(0, 200)}`,
      );
      ...
      if (error) {
        ...
        logger.error(
          "RecipeImportRepository",
          `Edge Function error [${error?.constructor?.name}]: ${detail}`,
        );
        throw new Error(
          "Não foi possível extrair a receita. Verifique o URL e tente novamente.",
        );
      }

      if (data?.error) {
        throw new Error(
          data.error === "No recipe found"
  ...
  async extractFromYoutube(
    ...
    try {
      ...
      if (error) {
            ...
            if (ctx.body) {
              const bodyText =
                typeof ctx.body === "string"
                  ? ctx.body
                  : await new Response(ctx.body).text().catch(() => "");
        ...
        logger.error(
          "RecipeImportRepository",
          `YouTube Edge Function error [${error?.constructor?.name}]: ${detail}`,
        );
        ...
        try {
          ...
          if (ctx?.body) {
            ...
            if (parsed?.error) {
              if (String(parsed.error).includes("API key not configured")) {
                throw new Error(
                  "YouTube API key não configurada. Contacte o administrador.",
                );
              }
              if (String(parsed.error).includes("No recipe found")) {
                throw new Error(
                  "Não foi encontrada nenhuma receita neste vídeo.",
                );
              }
          ...
          if (
            ...
            (parseErr.message.startsWith("Não foi") ||
              parseErr.message.startsWith("YouTube") ||
              parseErr.message.startsWith("Erro"))
          ) {
            throw parseErr;
          }
        }
        throw new Error(
          "Não foi possível extrair a receita do YouTube. Tente novamente.",
        );
      }
      ...
      if (
        ...
        (err.message.startsWith("Não foi") || err.message.startsWith("Erro na"))
      ) {
        throw err;
      }
  ...
  async extractFromPhoto(
    ...
    try {
      ...
      if (error) {
        logger.error(
          "RecipeImportRepository",
          "Photo Edge Function error",
          error,
        );
        throw new Error(
          "Não foi possível extrair a receita da foto. Tente com uma foto mais nítida.",
        );
      }
  ...
  async extractFromText(
    ...
    try {
      ...
      if (error) {
        logger.error(
          "RecipeImportRepository",
          "Text Edge Function error",
          error,
        );
        throw new Error(
          "Não foi possível extrair a receita do texto. Tente novamente.",
        );
      }
      ...
      if (data?.error) {
        ...
        if (msg.includes("No recipe found")) {
          throw new Error(
            "Não foi encontrada nenhuma receita no texto fornecido.",
          );
        }
        throw new Error(`Erro na extração: ${msg}`);
      }
