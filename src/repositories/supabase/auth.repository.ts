...
export class SupabaseAuthRepository implements IAuthRepository {
  ...
  async signInWithGoogle(): Promise<UserAccount> {
    ...
    if (authError || !authData.user) {
      ...
      logger.error("Auth", "signInWithIdToken failed", {
    ...
    const { data: accountRows, error: rpcError } = await this.client.rpc(
      ...
      {
        p_google_id:
          (authData.user.user_metadata?.["sub"] as string | undefined) ??
          authData.user.id,
    ...
    if (rpcError || !accountRows || accountRows.length === 0) {
      logger.error("Auth", "provision_user_account failed", {
        rpcError,
        accountRows,
      });
      throw new Error(
        `Erro ao criar conta: ${rpcError?.message ?? "Sem resposta"}`,
      );
    }
  ...
  async getCurrentSession(): Promise<UserAccount | null> {
    ...
    const { data: accountRow, error: accountError } = await this.client
      .from("user_accounts")
      .select(
        "id, google_id, email, family_id, profile_id, created_at, updated_at",
      )
    ...
    if (!accountRow.profile_id) {
      logger.warn(
        "Auth",
        "getCurrentSession: profile_id is NULL, attempting re-link",
      );
      ...
      const { data: accountRows, error: rpcError } = await this.client.rpc(
        ...
        {
          p_google_id:
            (user.user_metadata?.["sub"] as string | undefined) ?? user.id,
          p_email: user.email ?? "",
          p_display_name: displayName,
        },
      );
    ...
    if (profileRow?.status === "inactive") {
      logger.warn(
        "Auth",
        "getCurrentSession: profile inactive, blocking access",
        {
