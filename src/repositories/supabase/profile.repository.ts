...
export class SupabaseProfileRepository implements IProfileRepository {
  ...
  async createProfile(
    ...
    if (error || !data || data.length === 0) {
      ...
      throw new Error(
        `Erro ao criar perfil: ${error?.message ?? "Sem resposta"}`,
      );
    }
  ...
  async updateProfile(
    ...
    if (error || !rows || rows.length === 0) {
      logger.error("ProfileRepository", "updateProfile failed", {
        error,
        rowCount: rows?.length,
      });
      throw new Error(
        `Erro ao actualizar perfil: ${error?.message ?? "Perfil não encontrado"}`,
      );
    }
  ...
  async deleteProfile(id: string): Promise<void> {
    ...
    if (checkError) {
      logger.error(
        "ProfileRepository",
        "deleteProfile: check query failed",
        checkError,
      );
