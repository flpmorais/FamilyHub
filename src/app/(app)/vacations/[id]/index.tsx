...
import { mapPackingRow } from '../../../../repositories/supabase/packing-item.repository';
...
export default function VacationDetailScreen() {
  ...
  // Reactive packing items
  const { data: rawPackingRows } = useQuery(
    'SELECT * FROM packing_items WHERE vacation_id = ? ORDER BY created_at',
    [vacationId ?? '']
  );
  const items = useMemo(() => (rawPackingRows ?? []).map(mapPackingRow), [rawPackingRows]);
  ...
  useEffect(() => {
    if (!isLoading && !tabInitialized) {
      ...
      setActiveTab(isPacking || allDone ? 'packing' : 'tasks');
  ...
  async function handleLifecycleChange(lc: VacationLifecycle) {
    if (!vacation) return;
    try {
      await vacationRepo.updateVacation(vacation.id, { lifecycle: lc });
      await loadData();
    } catch (err) {
