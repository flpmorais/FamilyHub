...
import { View, Text, TouchableOpacity, StyleSheet, ActivityIndicator, FlatList, ScrollView, Modal, Alert, Switch } from 'react-native';
...
import type { Vacation, VacationLifecycle, BookingTask, VacationBag } from '../../../../types/vacation.types';
import { useIconStore } from '../../../../stores/icon.store';
import type { PackingStatus, Category, Tag, BagTemplate } from '../../../../types/packing.types';
...
export default function VacationDetailScreen() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [allVacs, taskList, profList, catList, tagList, packingList, vacBags, bagList] = await Promise.all([
  ...
  async function handleStatusChange(id: string, status: PackingStatus, vacationBagId?: string) {
  ...
  async function handleAddBag(bagTemplate: BagTemplate) {
    await vacationRepo.addVacationBag(vacationId!, bagTemplate.id, bagTemplate.isTopLevel);
  ...
  async function handleSaveBag() {
    ...
    if (editBagTopLevel !== editingBag.isTopLevel) {
      await vacationRepo.updateVacationBagTopLevel(editingBag.id, editBagTopLevel);
    }
    setEditingBag(null);
    await reloadVacationBags();
  }
  ...
  async function handleDeleteBagWithItems() {
    if (!editingBag) return;
    Alert.alert('Eliminar mala?', 'Os itens nesta mala ficarão sem mala atribuída e os embalados voltarão ao estado "Novo".', [
      { text: 'Cancelar', style: 'cancel' },
      {
        text: 'Eliminar',
        style: 'destructive',
        onPress: async () => {
          const bagItems = items.filter((i) => i.vacationBagId === editingBag.id);
        ...
        <View style={{ flex: 1 }}>
          <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 80 }}>
            {vacationBags.length === 0 && (
              <Text style={{ color: '#888888', textAlign: 'center', marginVertical: 32 }}>Nenhuma mala adicionada.</Text>
            )}
            {vacationBags.map((vb) => {
              ...
              return (
                <View key={vb.id} style={st.accordionContainer}>
                  <View style={st.accordionHeader}>
                    <TouchableOpacity
                      style={{ flex: 3, flexDirection: 'row', alignItems: 'center', gap: 10 }}
                      onPress={() => openEditBag(vb)}
                    >
                      <View style={[st.bagDot, { backgroundColor: bt.color }]} />
                      <View style={{ flex: 1 }}>
                        <Text style={st.bagName}>{bt.name} · {bagItems.length} {bagItems.length === 1 ? 'item' : 'itens'}</Text>
                        <Text style={st.bagMeta}>{bt.sizeLiters}L · {vb.isTopLevel ? 'Principal' : 'Interna'}</Text>
                      </View>
                    </TouchableOpacity>
                    <TouchableOpacity
                    ...
                    >
                      <Icon source={isExpanded ? 'chevron-up' : 'chevron-down'} size={24} color="#888888" />
                    </TouchableOpacity>
                  </View>
                  {isExpanded && (
                    <View style={st.accordionBody}>
                      {bagItems.length === 0 ? (
                        <Text style={st.accordionEmpty}>Sem itens nesta mala</Text>
                      ) : (
                        bagItems.map((item) => (
                          <View key={item.id} style={st.accordionItem}>
                            <Text style={st.accordionItemName} numberOfLines={1}>{item.name}</Text>
                            <Text style={st.accordionItemMeta}>
                              {item.assignedProfileId ? profiles.find((p) => p.id === item.assignedProfileId)?.displayName ?? '' : ''}
                            </Text>
                          </View>
                        ))
                      )}
              ...
              return (
                <View style={st.accordionContainer}>
                  <View style={st.accordionHeader}>
                    ...
                    <View style={{ flex: 1 }}>
                      <Text style={[st.bagName, { color: '#888888' }]}>Sem mala</Text>
                      <Text style={st.bagMeta}>{unassigned.length} {unassigned.length === 1 ? 'item' : 'itens'}</Text>
                    </View>
                  </View>
                  <View style={st.accordionBody}>
                    {unassigned.map((item) => (
          ...
          >
            <View style={st.bagPickerOverlay}>
              <TouchableOpacity style={{ flex: 1 }} activeOpacity={1} onPress={() => setBagPickerVisible(false)} />
              <View style={st.bagPickerSheet}>
                <Text style={st.bagPickerTitle}>Adicionar mala</Text>
                {(() => {
                  const addedIds = new Set(vacationBags.map((vb) => vb.bagTemplateId));
                  const available = bagTemplates.filter((b) => b.active && !addedIds.has(b.id));
                  if (available.length === 0) return <Text style={{ color: '#888888', marginBottom: 16 }}>Todas as malas já foram adicionadas.</Text>;
                  return (
                    <FlatList
                      data={available}
                      keyExtractor={(item) => item.id}
                      renderItem={({ item: bag }) => (
                        <TouchableOpacity style={st.bagPickerRow} onPress={() => handleAddBag(bag)}>
                          <View style={[st.bagDot, { backgroundColor: bag.color }]} />
                          <Text style={{ fontSize: 15, color: '#1A1A1A', flex: 1 }}>{bag.name}</Text>
                          <Text style={{ fontSize: 13, color: '#888888' }}>{bag.sizeLiters}L</Text>
                        </TouchableOpacity>
                      )}
                    />
                  );
          ...
          {/* Edit bag modal */}
          {editingBag && (() => {
            const bt = bagTemplates.find((b) => b.id === editingBag.bagTemplateId);
            return (
              <Modal
                visible
                animationType="slide"
              ...
              >
                <View style={st.bagPickerOverlay}>
                  <TouchableOpacity style={{ flex: 1 }} activeOpacity={1} onPress={() => setEditingBag(null)} />
                  <View style={st.bagPickerSheet}>
                    <Text style={st.bagPickerTitle}>Editar mala</Text>
                    {bt && (
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                        <View style={[st.bagDot, { backgroundColor: bt.color }]} />
                        <Text style={{ fontSize: 16, fontWeight: '500', color: '#1A1A1A' }}>{bt.name}</Text>
                        <Text style={{ fontSize: 13, color: '#888888' }}>{bt.sizeLiters}L</Text>
                      </View>
                    )}
                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, paddingVertical: 4 }}>
                      <Text style={{ fontSize: 15, color: '#1A1A1A' }}>Mala principal</Text>
                      <Switch
                    ...
                    <View style={{ flexDirection: 'row', gap: 12 }}>
                      <TouchableOpacity
                        style={{ paddingVertical: 14, paddingHorizontal: 12, borderRadius: 8, borderWidth: 1, borderColor: '#D32F2F', alignItems: 'center' }}
                        onPress={handleDeleteBagWithItems}
                      >
                        <Text style={{ color: '#D32F2F', fontSize: 14, fontWeight: '600' }}>Eliminar</Text>
                      </TouchableOpacity>
                      <TouchableOpacity
                        style={{ flex: 1, backgroundColor: '#B5451B', paddingVertical: 14, borderRadius: 8, alignItems: 'center' }}
                        onPress={handleSaveBag}
                      >
                        <Text style={{ color: '#FFFFFF', fontSize: 16, fontWeight: '600' }}>Guardar</Text>
                      </TouchableOpacity>
                    </View>
                  </View>
                </View>
...
const st = StyleSheet.create({
  ...
  // Bags tab
  accordionContainer: { marginBottom: 12, borderRadius: 8, borderWidth: 1, borderColor: '#F0F0F0', overflow: 'hidden' },
  accordionHeader: { flexDirection: 'row', alignItems: 'center', padding: 12, gap: 10, backgroundColor: '#FAFAFA' },
  accordionBody: { paddingHorizontal: 12, paddingBottom: 8 },
  accordionEmpty: { fontSize: 13, color: '#AAAAAA', fontStyle: 'italic', paddingVertical: 8 },
  accordionItem: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: '#F5F5F5' },
  ...
  topLevelChip: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12, borderWidth: 1, borderColor: '#CCCCCC' },
  ...
  bagFab: { position: 'absolute', bottom: 16, right: 16, width: 56, height: 56, borderRadius: 16, backgroundColor: '#B5451B', alignItems: 'center', justifyContent: 'center', elevation: 6 },
  bagFabText: { color: '#FFFFFF', fontSize: 28, fontWeight: '400', marginTop: -2 },
  bagPickerOverlay: { flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(0,0,0,0.4)' },
  bagPickerSheet: { backgroundColor: '#FFFFFF', borderTopLeftRadius: 16, borderTopRightRadius: 16, padding: 20, paddingBottom: 40, maxHeight: '50%' },
  bagPickerTitle: { fontSize: 18, fontWeight: '700', color: '#1A1A1A', marginBottom: 16 },
  bagPickerRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#F0F0F0', gap: 10 },
});
