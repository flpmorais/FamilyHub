...
const TagDraggableRow = memo(function TagDraggableRow({
  tag,
  onOpenEdit,
}: TagDraggableRowProps) {
  ...
  return (
    ...
    >
      ...
      <Text style={[s.rowName, !tag.active && s.rowNameInactive]}>
        {tag.name}
      </Text>
  ...
  async function handleSave(keepOpen: boolean = false) {
    ...
    try {
      ...
      if (editingTag) {
        ...
        if (editingTag.active !== formActive)
          await tagRepo.setActive(editingTag.id, formActive);
              ...
              >
                ...
                >
                  <Text style={s.filterApplyBtnText}>
                    Ver {filteredTags.length} etiquetas
                  </Text>
                </TouchableOpacity>
              </ScrollView>
            </View>
