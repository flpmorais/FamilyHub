...
const ShoppingCategoryDraggableRow = memo(
  function ShoppingCategoryDraggableRow({
    category,
    onOpenEdit,
  ...
  async function handleSave(keepOpen: boolean = false) {
    ...
    try {
      if (editingCat) {
        ...
        if (editingCat.active !== formActive)
          await catRepo.setActive(editingCat.id, formActive);
