...
export default function RecipeCategoriesScreen() {
  ...
  const catRepo = useRepository('recipeCategory');
  const { userAccount } = useAuthStore();

  const [categories, setCategories] = useState<RecipeCategory[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [sheetVisible, setSheetVisible] = useState(false);
  const [editingCat, setEditingCat] = useState<RecipeCategory | null>(null);
  ...
  async function loadCategories() {
    ...
    try {
      ...
      logger.error('RecipeCategoriesScreen', 'load failed', err);
    } finally {
      setIsLoading(false);
    }
  }
  ...
  async function handleSave(keepOpen: boolean = false) {
    ...
    try {
      if (editingCat) {
        ...
        await catRepo.create({ familyId: userAccount!.familyId, name });
      ...
      logger.error('RecipeCategoriesScreen', 'save failed', err);
    } finally {
      setIsSaving(false);
    }
  }

  async function handleDelete(cat: RecipeCategory) {
    const count = await catRepo.countRecipesUsingCategory(cat.id);
    if (count > 0) {
      Alert.alert(
        'Não é possível eliminar',
        `Esta categoria está a ser utilizada por ${count} receita${count === 1 ? '' : 's'}. Elimine as associações primeiro.`,
      );
      return;
    }
  ...
  return (
    <View style={s.container}>
      <PageHeader title="Categorias de Receitas" showBack familyBannerUri={family?.bannerUrl} />

      <FlatList
        data={categories}
        keyExtractor={(item) => item.id}
        renderItem={({ item: cat }) => (
          <TouchableOpacity style={s.row} onPress={() => openEdit(cat)}>
            <Text style={s.rowName}>{cat.name}</Text>
          </TouchableOpacity>
        )}
        ListEmptyComponent={<Text style={s.empty}>Nenhuma categoria criada.</Text>}
