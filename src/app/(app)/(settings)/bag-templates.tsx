...
const BAG_COLORS = [
  '#D32F2F', '#C2185B', '#9C27B0', '#6A1B9A', '#3F51B5',
  '#1976D2', '#03A9F4', '#00897B', '#009688', '#388E3C',
...
export default function BagTemplatesScreen() {
  ...
  async function handleDelete(bag: BagTemplate) {
    ...
    Alert.alert(`Eliminar "${bag.name}"?`, 'Esta acção não pode ser desfeita.', [
            ...
            <View style={s.filterPanel}>
              <ScrollView showsVerticalScrollIndicator={false}>
                <View style={s.filterPanelHeader}>
                  ...
                  {filterCount > 0 && (
                    <TouchableOpacity
                      onPress={() => { setShowActiveOnly(false); setSearchText(''); }}
                    >
                ...
                <TouchableOpacity style={s.filterApplyBtn} onPress={() => setFilterPanelVisible(false)}>
                  <Text style={s.filterApplyBtnText}>Ver {filteredBags.length} malas</Text>
                </TouchableOpacity>
              </ScrollView>
            </View>
          </View>
