...
export default function MealPlanScreen() {
  ...
  const [dishesMap, setDishesMap] = useState<Map<string, MealEntryDish[]>>(
    new Map(),
  );
  ...
  const loadWeek = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const data = await mealPlanRepo.getWeek(
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((err: unknown) => {
        logger.error(
          "MealPlanScreen",
          "Erro ao carregar perfis/configuração",
          err,
        );
      });
    supabaseClient
  ...
  function isSlotSkippedByConfig(
    dayOfWeek: number,
    mealSlot: MealSlot,
  ): boolean {
    const config = slotConfigs.find(
      (c) => c.dayOfWeek === dayOfWeek && c.mealSlot === mealSlot,
    );
  ...
  async function handleLeftoverSave(
    items: {
      name: string;
      type: LeftoverType;
      totalDoses: number;
    ...
    try {
      ...
      setSnackbarMsg(
        `${items.length} resto${items.length > 1 ? "s" : ""} adicionado${items.length > 1 ? "s" : ""}`,
      );
  ...
  const [pendingDishForAdd, setPendingDishForAdd] =
    useState<CreateDishInput | null>(null);
  ...
  async function handleGenerateShoppingList() {
    ...
    try {
      ...
      const uniqueRecipeIds = [
        ...new Set(allRecipeDishes.map((d) => d.recipeId)),
      ];
  ...
  const getEntry = (
    ...
    return entries.find(
      (e) => e.dayOfWeek === dayOfWeek && e.mealSlot === slot,
    );
  };
  ...
  function renderSlotRow(day: number, slot: MealSlot) {
    ...
    return (
      <TouchableOpacity
        ...
        activeOpacity={0.6}
      >
        <Text style={[styles.slotLabel, skipped && styles.slotLabelSkipped]}>
          {SLOT_LABELS[slot]}
        </Text>
  ...
  return (
    <View style={styles.container}>
              ...
              >
                <TouchableOpacity
                  style={[
                    ...
                    today && styles.dayCardHeaderToday,
                  ]}
                ...
                >
                  <Text
                    style={[
                      styles.dayCardTitle,
                      today && styles.dayCardTitleToday,
                    ]}
                  >
      ...
      <MealAddForm
        ...
        defaultParticipants={
          addSlot
            ? getDefaultParticipants(addSlot.dayOfWeek, addSlot.mealSlot)
            : []
        }
        onClose={() => {
          setAddSlot(null);
          setPendingDishForAdd(null);
          loadWeek();
        }}
      ...
      <MealEditForm
        ...
        familyId={userAccount?.familyId ?? ""}
        mealDate={
          editMeal ? getMealDate(currentWeekStart, editMeal.dayOfWeek) : ""
        }
        profiles={profiles}
