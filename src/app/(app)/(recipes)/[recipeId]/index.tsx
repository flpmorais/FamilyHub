...
export default function RecipeDetailScreen() {
  ...
  const [ratingSummary, setRatingSummary] = useState<RecipeRatingSummary>({
    average: null,
    count: 0,
  });
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === "DELETE") {
            logger.info(
              "RecipeDetailScreen",
              "recipe deleted by another admin",
            );
            setRecipe(null);
            ...
            logger.info(
              "RecipeDetailScreen",
              "recipe updated by another admin",
            );
  ...
  async function handleShare() {
    ...
    try {
      ...
      setErrorMsg(
        "Não foi possível partilhar a receita. Requer um development build.",
      );
      setErrorVisible(true);
    } finally {
      setIsSharing(false);
    ...
    try {
      const items = generateShoppingList(
        [
          {
            recipeId: recipe.id,
            servingsOverride: targetServings || recipe.servings,
          },
        ],
    ...
    <ScrollView style={s.container}>
      ...
      <View style={s.floatingRight}>
        ...
        >
          <Text style={s.floatingPillText}>
            {isSharing ? "A gerar..." : "Partilhar 📤"}
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={s.floatingPill}
          onPress={() =>
            router.push(`/(app)/(recipes)/${recipeId}/edit` as any)
          }
        >
      ...
      <View style={s.content}>
        ...
        <View style={s.titleRow}>
          <Text style={[s.name, { marginBottom: 0, flex: 1 }]}>
            {recipe.name}
          </Text>
          <TouchableOpacity
        ...
        {recipe.cost && <Text style={s.cost}>Custo: {recipe.cost}</Text>}
