...
export default function ImportReviewScreen() {
  ...
  const extracted: ExtractedRecipe | null = (() => {
    try {
      return extractedJson ? JSON.parse(extractedJson) : null;
    } catch {
      return null;
    }
  })();
  ...
  useEffect(() => {
    ...
    setIngredients(
      extracted.ingredients.length > 0
        ? extracted.ingredients.map((i) => ({ name: i.name, quantity: i.quantity ?? '' }))
        : [{ name: '', quantity: '' }],
    );
    setSteps(
      extracted.steps.length > 0
        ? extracted.steps.map((s) => ({ text: s }))
        : [{ text: '' }],
    );
  ...
  async function handleInlineCreateCategory() {
    ...
    try {
      const newCat = await recipeCategoryRepo.create({ familyId, name: inlineName.trim() });
      setAllCategories([...allCategories, newCat]);
      setSelectedCategoryIds([...selectedCategoryIds, newCat.id]);
      setInlineName('');
      setInlineCategoryModal(false);
    } catch (err) {
      logger.error('ImportReviewScreen', 'inline category create failed', err);
  ...
  async function handleInlineCreateTag() {
    if (!familyId || !inlineName.trim()) return;
    try {
      const newTag = await recipeTagRepo.create({ familyId, name: inlineName.trim() });
      setAllTags([...allTags, newTag]);
      setSelectedTagIds([...selectedTagIds, newTag.id]);
      setInlineName('');
      setInlineTagModal(false);
    } catch (err) {
      logger.error('ImportReviewScreen', 'inline tag create failed', err);
  ...
  async function handleSave() {
    ...
    try {
      await recipeRepo.create({
        ...
        importMethod: (importMethodParam as any) ?? 'url',
      ...
      // Navigate back to recipe list (skip import-url from stack)
      router.dismiss(2);
    } catch (err) {
      logger.error('ImportReviewScreen', 'save failed', err);
  ...
  if (!extracted) {
    return (
      <View style={s.centered}>
        <Text style={s.errorText}>Dados de importação inválidos</Text>
  ...
  return (
    ...
    >
      <View style={s.header}>
        ...
        <Text style={s.headerTitle}>Rever Receita</Text>
        <View style={{ width: 60 }} />
      </View>
      ...
      <ScrollView style={s.flex} contentContainerStyle={s.form}>
        ...
        <TextInput style={s.input} value={name} onChangeText={setName} placeholder="Nome da receita" placeholderTextColor="#CCCCCC" />

        {/* Type */}
        <Text style={s.label}>Tipo *</Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.typeScroll}>
          {RECIPE_TYPE_LIST.map((t) => (
            <TouchableOpacity key={t.key} style={[s.typeChip, type === t.key && s.typeChipActive]} onPress={() => setType(t.key)}>
        ...
        <View style={s.row}>
          <View style={s.rowItem}>
            ...
            <TextInput style={s.input} value={servings} onChangeText={setServings} keyboardType="numeric" placeholder="4" placeholderTextColor="#CCCCCC" />
          </View>
          <View style={s.rowItem}>
            <Text style={s.label}>Prep (min)</Text>
            <TextInput style={s.input} value={prepTime} onChangeText={setPrepTime} keyboardType="numeric" placeholder="15" placeholderTextColor="#CCCCCC" />
          </View>
          <View style={s.rowItem}>
            <Text style={s.label}>Cozinhar (min)</Text>
            <TextInput style={s.input} value={cookTime} onChangeText={setCookTime} keyboardType="numeric" placeholder="30" placeholderTextColor="#CCCCCC" />
          </View>
        </View>

        {/* Cost */}
        <Text style={s.label}>Custo</Text>
        <TextInput style={s.input} value={cost} onChangeText={setCost} placeholder="Ex: €5" placeholderTextColor="#CCCCCC" />
        ...
        {imageUri ? (
          <View style={s.imagePreviewContainer}>
            ...
            <TouchableOpacity style={s.imageRemove} onPress={() => setImageUri(null)}>
        ...
        <View style={s.chipContainer}>
          {allCategories.map((cat) => (
            <TouchableOpacity key={cat.id} style={[s.chip, selectedCategoryIds.includes(cat.id) && s.chipSelected]} onPress={() => toggleCategory(cat.id)}>
              <Text style={[s.chipText, selectedCategoryIds.includes(cat.id) && s.chipTextSelected]}>{cat.name}</Text>
            </TouchableOpacity>
          ))}
          <TouchableOpacity style={s.chipAdd} onPress={() => { setInlineName(''); setInlineCategoryModal(true); }}>
        ...
        <View style={s.chipContainer}>
          {allTags.map((tag) => (
            <TouchableOpacity key={tag.id} style={[s.chip, selectedTagIds.includes(tag.id) && s.chipSelected]} onPress={() => toggleTag(tag.id)}>
              <Text style={[s.chipText, selectedTagIds.includes(tag.id) && s.chipTextSelected]}>{tag.name}</Text>
            </TouchableOpacity>
          ))}
          <TouchableOpacity style={s.chipAdd} onPress={() => { setInlineName(''); setInlineTagModal(true); }}>
            <Text style={s.chipAddText}>+ Nova</Text>
          </TouchableOpacity>
        </View>
        ...
        {ingredients.map((ing, i) => (
          <View key={i} style={s.dynamicRow}>
            <View style={s.dynamicRowInputs}>
              <TextInput style={[s.input, s.ingredientName]} value={ing.name} onChangeText={(v) => updateIngredient(i, 'name', v)} placeholder="Ingrediente" placeholderTextColor="#CCCCCC" />
              <TextInput style={[s.input, s.ingredientQty]} value={ing.quantity} onChangeText={(v) => updateIngredient(i, 'quantity', v)} placeholder="Qtd" placeholderTextColor="#CCCCCC" />
            </View>
            <View style={s.dynamicRowActions}>
              {i > 0 && <TouchableOpacity onPress={() => moveIngredient(i, 'up')}><Text style={s.actionBtn}>↑</Text></TouchableOpacity>}
              {i < ingredients.length - 1 && <TouchableOpacity onPress={() => moveIngredient(i, 'down')}><Text style={s.actionBtn}>↓</Text></TouchableOpacity>}
              {ingredients.length > 1 && <TouchableOpacity onPress={() => removeIngredient(i)}><Text style={s.removeBtn}>✕</Text></TouchableOpacity>}
            </View>
          </View>
        ))}
        ...
        {steps.map((step, i) => (
          <View key={i} style={s.dynamicRow}>
            <View style={s.stepNumberContainer}><Text style={s.stepNumber}>{i + 1}.</Text></View>
            <TextInput style={[s.input, s.stepInput]} value={step.text} onChangeText={(v) => updateStep(i, v)} placeholder={`Passo ${i + 1}`} placeholderTextColor="#CCCCCC" multiline />
            <View style={s.dynamicRowActions}>
              {i > 0 && <TouchableOpacity onPress={() => moveStep(i, 'up')}><Text style={s.actionBtn}>↑</Text></TouchableOpacity>}
              {i < steps.length - 1 && <TouchableOpacity onPress={() => moveStep(i, 'down')}><Text style={s.actionBtn}>↓</Text></TouchableOpacity>}
              {steps.length > 1 && <TouchableOpacity onPress={() => removeStep(i)}><Text style={s.removeBtn}>✕</Text></TouchableOpacity>}
            </View>
          </View>
        ))}
        ...
        <TouchableOpacity style={[s.saveBtn, isSaving && s.saveBtnDisabled]} onPress={handleSave} disabled={isSaving}>
          {isSaving ? <ActivityIndicator color="#FFFFFF" /> : <Text style={s.saveBtnText}>Guardar Receita</Text>}
        </TouchableOpacity>
      ...
      <Modal visible={inlineCategoryModal} animationType="slide" transparent onRequestClose={() => setInlineCategoryModal(false)}>
        <View style={s.inlineModalOverlay}>
          <View style={s.inlineSheet}>
            <Text style={s.inlineSheetTitle}>Nova Categoria</Text>
            <TextInput style={s.input} value={inlineName} onChangeText={setInlineName} placeholder="Nome da categoria" placeholderTextColor="#CCCCCC" autoCapitalize="sentences" autoFocus />
            <View style={s.inlineSheetBtns}>
              <TouchableOpacity style={s.inlineCancelBtn} onPress={() => setInlineCategoryModal(false)}><Text style={s.inlineCancelText}>Cancelar</Text></TouchableOpacity>
              <TouchableOpacity style={s.inlineSaveBtn} onPress={handleInlineCreateCategory}><Text style={s.inlineSaveText}>Criar</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>
      ...
      {/* Inline Tag Modal */}
      <Modal visible={inlineTagModal} animationType="slide" transparent onRequestClose={() => setInlineTagModal(false)}>
        <View style={s.inlineModalOverlay}>
          <View style={s.inlineSheet}>
            <Text style={s.inlineSheetTitle}>Nova Etiqueta</Text>
            <TextInput style={s.input} value={inlineName} onChangeText={setInlineName} placeholder="Nome da etiqueta" placeholderTextColor="#CCCCCC" autoCapitalize="sentences" autoFocus />
            <View style={s.inlineSheetBtns}>
              <TouchableOpacity style={s.inlineCancelBtn} onPress={() => setInlineTagModal(false)}><Text style={s.inlineCancelText}>Cancelar</Text></TouchableOpacity>
              <TouchableOpacity style={s.inlineSaveBtn} onPress={handleInlineCreateTag}><Text style={s.inlineSaveText}>Criar</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>

      <Snackbar visible={errorVisible} onDismiss={() => setErrorVisible(false)} duration={3000} style={s.snackbar} theme={{ colors: { inverseSurface: '#D32F2F', inverseOnSurface: '#FFFFFF' } }}>
        {errorMsg}
      </Snackbar>
    </KeyboardAvoidingView>
  );
...
const s = StyleSheet.create({
  ...
  errorText: { fontSize: 16, color: '#888888', marginBottom: 16 },
  ...
  input: { borderWidth: 1, borderColor: '#E0E0E0', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10, fontSize: 15, color: '#1A1A1A' },
  ...
  imageRemove: { position: 'absolute', top: 8, right: 8, width: 28, height: 28, borderRadius: 14, backgroundColor: 'rgba(0,0,0,0.5)', alignItems: 'center', justifyContent: 'center' },
