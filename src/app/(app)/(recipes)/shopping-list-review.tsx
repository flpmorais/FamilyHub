...
export default function ShoppingListReviewScreen() {
  ...
  async function handleAddToShoppingList() {
    ...
    try {
      ...
      const parts: string[] = [];
      if (created > 0)
        parts.push(`${created} adicionado${created > 1 ? "s" : ""}`);
      if (updated > 0)
        parts.push(`${updated} atualizado${updated > 1 ? "s" : ""}`);
      const msg = parts.join(", ") || "Lista atualizada";

      Alert.alert("Lista de compras atualizada", msg, [
    ...
    <View style={s.container}>
      ...
      <FlatList
        ...
        renderItem={({ item, index }) => (
          <TouchableOpacity style={s.itemRow} onPress={() => toggleItem(index)}>
            ...
            <Text
              style={[s.itemName, item.checked && s.itemNameChecked]}
              numberOfLines={1}
            >
              {item.ingredientName}
            </Text>
      ...
      <View style={s.footer}>
        <TouchableOpacity
          style={[
            s.addBtn,
            (checkedCount === 0 || isMerging) && s.addBtnDisabled,
          ]}
