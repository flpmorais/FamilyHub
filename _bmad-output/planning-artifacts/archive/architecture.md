...
The Pi hosts two services in Podman containers:

**Session Service (HTTP REST, port 3000):**

| Endpoint | Method | Purpose |
|---|---|---|
| `/session/start` | POST | `?userId=X&skill=Y` — starts OpenCode process under user's home directory with specified Fluent skill |
| `/session/resume` | POST | `?userId=X` — reconnects to existing OpenCode process |
| `/session/end` | POST | `?userId=X` — terminates OpenCode process |
| `/session/status` | GET | `?userId=X` — returns `{ active: bool, skill: string }` |
| `/auth/configure` | POST | `?userId=X` — validates API key, writes to per-user auth.json, provisions Linux account + home directory + OpenCode + Fluent skill files |

Implementation: Express.js in Podman container. Each endpoint manages OpenCode processes under the target Linux user. User mapping: `filipe` → Linux user `filipe`, `angela` → Linux user `angela`. API key is written to `~{userId}/.local/share/opencode/auth.json` with file permissions restricted to the owning user only (NFR32).

**WebSocket Server (port 3001):**

Accepts connections at `wss://ws.fh-morais.party?userId=X` (Cloudflare Tunnel). Routes messages only to the matching userId — zero cross-talk (FR104). OpenCode's Fluent skill sends messages to this server via `/speak` endpoint on the session service. The server relays them to the correct phone.

Implementation: `ws` library (bare WebSocket, no Socket.io overhead).

**Process Management:** Podman with auto-restart on boot. Both services in a single pod or separate containers with `--restart=always`.

**Networking:** Cloudflare Tunnel. `api.fh-morais.party` → session service (port 3000). `ws.fh-morais.party` → WebSocket server (port 3001). No VPN required on phone, no public IP, TLS terminated by Cloudflare.
...
Harness code lives in `harness/` within the FamilyHub repo — different runtime (Python/FastAPI on Raspberry Pi), different deployment (Podman), different concerns. The mobile app treats the harness as an external service.
...
- Harness code lives in `harness/` within the FamilyHub repo, deployed separately via Podman
