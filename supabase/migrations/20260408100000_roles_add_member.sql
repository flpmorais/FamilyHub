-- Add 'member' role, remap existing admin profiles to member, keep Filipe as admin.
-- Update provision_user_account() so new first-time users get 'member' instead of 'admin'.

-- ─── 1. Insert member role ────────────────────────────────────────────────────
INSERT INTO roles (id, name) VALUES ('member', 'Membro') ON CONFLICT (id) DO NOTHING;

-- ─── 2. Remap all admin profiles → member, then set Filipe back to admin ──────
UPDATE profiles SET role = 'member' WHERE role = 'admin';
UPDATE profiles SET role = 'admin' WHERE id = '00000000-0000-0000-0001-000000000001';

-- ─── 3. Recreate provision_user_account() with member fallback ────────────────
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM user_accounts ua WHERE ua.id = auth.uid()) THEN
    ...
    -- Re-link if profile_id is NULL (e.g. after data reseed)
    IF EXISTS (SELECT 1 FROM user_accounts ua WHERE ua.id = auth.uid() AND ua.profile_id IS NULL) THEN
      SELECT p.id
      INTO v_profile_id
      FROM profiles p
      JOIN user_accounts ua ON ua.family_id = p.family_id
      WHERE ua.id = auth.uid()
      ...
      IF v_profile_id IS NOT NULL THEN
        UPDATE profiles SET status = 'enrolled' WHERE profiles.id = v_profile_id;
        UPDATE user_accounts SET profile_id = v_profile_id WHERE user_accounts.id = auth.uid();
      ELSE
        RAISE EXCEPTION 'Perfil não encontrado para esta conta';
      END IF;
    END IF;
  ...
  -- Case 3: First-time fallback — assign 'member' role (not admin)
  ...
  IF v_family_id IS NOT NULL AND NOT EXISTS (
    ...
    IF v_profile_id IS NOT NULL THEN
      UPDATE profiles SET status = 'enrolled', email = p_email, role = 'member'
